...
**Anchor:** `src/conversation/orchestrator/core.ts:153` — `type Awaiting =`
...
**Anchor:** `src/conversation/orchestrator/core.ts:207` — `interface RunState {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:248` — `adjustNote: { parentKey: PathKey; note: string } | null;`
...
**Anchor:** `src/conversation/orchestrator/core.ts:507` — `if (eff.id.startsWith("resumed:")) {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:706` — `if (active) {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:772` — `const clearTurnWatchdog = (): void => {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:1015` — `const rearmed = pause.awaitingVariant;`
...
**Anchor:** `src/conversation/orchestrator/core.ts:1296` — `case "sizer": {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2145` — `const enqueueIngest = (work: () => Promise<void>): Promise<void> => {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2187` — `if (active) return false;`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2240` — `run.awaiting = { tag: "intent", buffer: "" };`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2599` — `run.prototypeRound++;`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2740` — `quotaPaused: () => run.quotaPause !== null || run.quotaPausePending,`
...
**Anchor:** `src/conversation/plan-tree/model.ts:91` — `export type NodeState =`
...
**Anchor:** `src/conversation/plan-tree/select.ts:71` — `export function writePolicyFor2(root: TreeNode): WritePolicy {`
...
**Anchor:** `src/main.ts:145` — `type ActionInFlight = "none" | "submit" | "approve";`
...
**Anchor:** `src/main.ts:232` — `type Selection =`
...
**Anchor:** `src/main.ts:244` — `function openPath(): AbsPath | null {`
...
**Anchor:** `src/main.ts:304` — `function placeholderSelected(): boolean {`
...
**Anchor:** `src/main.ts:356` — `function pendingSurfaces(): PendingSurface[] {`
...
**Anchor:** `src/main.ts:547` — `pendingCount: pendingSurfaces().length,`
...
**Anchor:** `src/main.ts:559` — `submitInFlight: actionInFlight === "submit",`
...
**Anchor:** `src/main.ts:853` — `refreshAffordances();`
...
**Anchor:** `src/main.ts:1376` — `function refreshAffordances(): void {`
...
**Anchor:** `src/main.ts:1699` — `function resolveSelection(`
...
**Anchor:** `src/main.ts:1708` — `if (heldGatePlan !== null && prev.path === heldGatePlan) return prev;`
...
**Anchor:** `src/main.ts:1726` — `if (isInitial(listState)) {`
...
**Anchor:** `src/main.ts:1749` — `console.error("list_plans failed — leaving the sidebar/selection intact", e);`
...
**Anchor:** `src/main.ts:2078` — `selection = isResumeSentinel(path)`
...
**Anchor:** `src/main.ts:2236` — `invalidatePopover(readingPaneEl);`
...
**Anchor:** `src/main.ts:2240` — `await settle(readingPaneEl, undefined, () => renderGuard.isCurrent(gen));`
...
**Anchor:** `src/main.ts:2433` — `async function openGatePlanFile(planPath: string): Promise<void> {`
...
**Anchor:** `src/main.ts:2454` — `const gateSurface = surfaces.find((s) => s.kind === "orchestrator-gate");`
...
**Anchor:** `src/main.ts:2472` — `const op = openPath();`
...
**Anchor:** `src/main.ts:2486` — `if (op !== null && changedPath === op && !isResumeSentinel(op)) {`
...
**Anchor:** `src/main.ts:2496` — `const opAfter = openPath();`
...
**Anchor:** `src/main.ts:2907` — `if (actionInFlight !== "none") return;`
...
**Anchor:** `src/main.ts:2920` — `actionInFlight = "submit"; // lock BEFORE the first await; reset in finally on EVERY exit.`
...
**Anchor:** `src/main.ts:2948` — `actionInFlight = "none";`
...
**Anchor:** `src/resume-banner.ts:37` — `export function computeAffordance(signals: {`

...
**Anchor:** `src/conversation/orchestrator/core.ts:161` — `type Awaiting =`
...
**Anchor:** `src/conversation/orchestrator/core.ts:222` — `interface RunState {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:263` — `adjustNote: { parentKey: PathKey; note: string } | null;`
...
**Anchor:** `src/conversation/orchestrator/core.ts:538` — `if (eff.id.startsWith("resumed:")) {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:747` — `if (active) {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:821` — `const clearTurnWatchdog = (): void => {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:1078` — `const rearmed = pause.awaitingVariant;`
...
**Anchor:** `src/conversation/orchestrator/core.ts:1374` — `case "sizer": {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2284` — `const enqueueIngest = (work: () => Promise<void>): Promise<void> => {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2328` — `if (active) return false;`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2387` — `run.awaiting = { tag: "intent", buffer: "" };`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2762` — `run.prototypeRound++;`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2909` — `quotaPaused: () => run.quotaPause !== null || run.quotaPausePending,`
...
**Anchor:** `src/conversation/plan-tree/model.ts:95` — `export type NodeState =`
...
**Anchor:** `src/conversation/plan-tree/select.ts:72` — `export function writePolicyFor2(root: TreeNode): WritePolicy {`
...
**Anchor:** `src/main.ts:164` — `type ActionInFlight = "none" | "submit" | "approve";`
...
**Anchor:** `src/main.ts:281` — `type Selection =`
...
**Anchor:** `src/main.ts:295` — `function openPath(): AbsPath | null {`
...
**Anchor:** `src/main.ts:381` — `function placeholderSelected(): boolean {`
...
**Anchor:** `src/main.ts:454` — `function pendingSurfaces(): PendingSurface[] {`
...
**Anchor:** `src/main.ts:748` — `pendingCount: pendingSurfaces().length,`
...
**Anchor:** `src/main.ts:760` — `submitInFlight: actionInFlight === "submit",`
...
**Anchor:** `src/main.ts:1054` — `refreshAffordances();`
...
**Anchor:** `src/main.ts:1577` — `function refreshAffordances(): void {`
...
**Anchor:** `src/main.ts:1900` — `function resolveSelection(`
...
**Anchor:** `src/main.ts:1909` — `if (heldGatePlan !== null && prev.path === heldGatePlan) return prev;`
...
**Anchor:** `src/main.ts:1927` — `if (isInitial(listState)) {`
...
**Anchor:** `src/main.ts:1950` — `console.error("list_plans failed — leaving the sidebar/selection intact", e);`
...
**Anchor:** `src/main.ts:2279` — `selection = isResumeSentinel(path)`
...
**Anchor:** `src/main.ts:2437` — `invalidatePopover(readingPaneEl);`
...
**Anchor:** `src/main.ts:2441` — `await settle(readingPaneEl, undefined, () => renderGuard.isCurrent(gen));`
...
**Anchor:** `src/main.ts:2634` — `async function openGatePlanFile(planPath: string): Promise<void> {`
...
**Anchor:** `src/main.ts:2655` — `const gateSurface = surfaces.find((s) => s.kind === "orchestrator-gate");`
...
**Anchor:** `src/main.ts:2673` — `const op = openPath();`
...
**Anchor:** `src/main.ts:2687` — `if (op !== null && changedPath === op && !isResumeSentinel(op)) {`
...
**Anchor:** `src/main.ts:2697` — `const opAfter = openPath();`
...
**Anchor:** `src/main.ts:3108` — `if (actionInFlight !== "none") return;`
...
**Anchor:** `src/main.ts:3121` — `actionInFlight = "submit"; // lock BEFORE the first await; reset in finally on EVERY exit.`
...
**Anchor:** `src/main.ts:3149` — `actionInFlight = "none";`
...
**Anchor:** `src/resume-banner.ts:59` — `export function computeAffordance(signals: {`

...
**Anchor:** `src/conversation/orchestrator/core.ts:1174` — `const rearmed = pause.awaitingVariant;`
...
**Anchor:** `src/conversation/orchestrator/core.ts:1463` — `case "sizer": {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2272` — `const enqueueIngest = (work: () => Promise<void>): Promise<void> => {`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2314` — `if (active) return false;`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2389` — `run.awaiting = { tag: "intent", buffer: "" };`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2761` — `run.prototypeRound++;`
...
**Anchor:** `src/conversation/orchestrator/core.ts:2907` — `quotaPaused: () => run.quotaPause !== null || run.quotaPausePending,`
...
**Anchor:** `src/main.ts:263` — `type Selection =`
...
**Anchor:** `src/main.ts:275` — `function openPath(): AbsPath | null {`
...
**Anchor:** `src/main.ts:346` — `function placeholderSelected(): boolean {`
...
**Anchor:** `src/main.ts:446` — `function pendingSurfaces(): PendingSurface[] {`
...
**Anchor:** `src/main.ts:640` — `pendingCount: pendingSurfaces().length,`
...
**Anchor:** `src/main.ts:652` — `submitInFlight: actionInFlight === "submit",`
...
**Anchor:** `src/main.ts:957` — `refreshAffordances();`
...
**Anchor:** `src/main.ts:1479` — `function refreshAffordances(): void {`
...
**Anchor:** `src/main.ts:1854` — `function resolveSelection(`
...
**Anchor:** `src/main.ts:1863` — `if (heldGatePlan !== null && prev.path === heldGatePlan) return prev;`
...
**Anchor:** `src/main.ts:1881` — `if (isInitial(listState)) {`
...
**Anchor:** `src/main.ts:1902` — `console.error("list_plans failed — leaving the sidebar/selection intact", e);`
...
**Anchor:** `src/main.ts:2453` — `selection = isResumeSentinel(path)`
...
**Anchor:** `src/main.ts:2611` — `invalidatePopover(readingPaneEl);`
...
**Anchor:** `src/main.ts:2615` — `await settle(readingPaneEl, undefined, () => renderGuard.isCurrent(gen));`
...
**Anchor:** `src/main.ts:2810` — `async function openGatePlanFile(planPath: string): Promise<void> {`
...
**Anchor:** `src/main.ts:2831` — `const gateSurface = surfaces.find((s) => s.kind === "orchestrator-gate");`
...
**Anchor:** `src/main.ts:2849` — `const op = openPath();`
...
**Anchor:** `src/main.ts:2863` — `if (op !== null && changedPath === op && !isResumeSentinel(op)) {`
...
**Anchor:** `src/main.ts:2873` — `const opAfter = openPath();`
...
**Anchor:** `src/main.ts:3391` — `if (actionInFlight !== "none") return;`
...
**Anchor:** `src/main.ts:3404` — `actionInFlight = "submit"; // lock BEFORE the first await; reset in finally on EVERY exit.`
...
**Anchor:** `src/main.ts:3435` — `actionInFlight = "none";`

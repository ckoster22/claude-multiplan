...
//
// Events carry full NodePaths. The gen-1 nn-addressed events map 1:1: RECON_DONE/SUB_RECON_DONE →
// NODE_RECON_DONE; MASTER_DRAFTED → DECOMPOSITION_DRAFTED; SUBPLANS_PARSED → CHILDREN_PARSED;
// MASTER_APPROVED → DECOMPOSITION_APPROVED; SUB_DRAFTED → NODE_DRAFTED;
// APPROVE/REQUEST_CHANGES/EXEC_DONE/SUMMARY_WRITTEN keep their names path-addressed; the master
// REQUEST_CHANGES (gen-1 driver-side only) becomes the first-class DECOMPOSITION_CHANGES_REQUESTED.
//
// DRIVER-WRITE BOUNDARY (cutover seam): gen-2 events carry NO plan/recon TEXT. The driver writes the
// artifact FIRST and dispatches the event with the write's real returned paths (generalizing the
// gen-1 SUMMARY_WRITTEN precedent). Consequently Effect2 has NO writeAgentPlan kind, and
// NODE_RECON_DONE emits no recon.md write (the driver writes recon.md itself before dispatching).
export type PlanTreeEvent2 =
  ...
  // INV-3 — THE RESUME PHASE-ONLY RE-ARM. On resume the disk-probe branch re-presents a decomposition
  ...
  // PHASE 5 — THE FORCED ACCEPTANCE GATE RESOLUTIONS (root-only, no path). Both perform the finalize
  ...
  // re-completion (baseline_ present, acceptance_ absent) the Phase-5 gate RE-ARMS. Clears
...
//
// Same effect KINDS and per-event ordering as gen 1 at depth 1, with deliberate driver-cutover deltas
// (documented on PlanTreeEvent2 above):
...
//   - notifyAwaitingApproval now fires for DECOMPOSITION_DRAFTED too (gate unification — the reducer
//     owns the former nn:-1-sentinel master gate now).
export type Effect2 =

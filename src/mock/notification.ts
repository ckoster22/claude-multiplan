...
//
// src/notify.ts posts desktop notifications via this plugin in EXACTLY two cases (quota
...
// the notification fires end-to-end through the real index.ts quota observer + real notify.ts.
// sendNotification still cannot throw into notify.ts (which swallows failures regardless), so the
// quota flow is safe.
